    ...
    <>
      ...
      <div className={`
        ...
        ${isOpen 
          ...
          : '-translate-x-full opacity-0 w-64 md:translate-x-0 md:w-0 md:opacity-0 md:border-r-0 pointer-events-none'
        }
      `}>
        {/* Fixed-width wrapper to prevent horizontal squeezing/wrapping during transition */}
        <div className="w-64 flex flex-col h-full shrink-0">
          <div className="p-4 flex items-center justify-between border-b border-slate-200 dark:border-slate-700">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-semibold text-lg whitespace-nowrap">DocumentDoc</h2>
            </div>
            <button 
              onClick={onClose}
          ...
          <div className="p-4 flex-1 overflow-y-auto space-y-6">
            ...
            <button
              ...
              className="w-full py-2.5 px-4 bg-brand-500 hover:bg-brand-600 text-white font-medium rounded-xl flex items-center justify-center gap-2 transition-colors shadow-sm text-sm whitespace-nowrap"
            >
            ...
            <div>
              <h3 className="text-xs font-semibold text-slate-400 dark:text-slate-500 mb-2 uppercase tracking-wider whitespace-nowrap">Knowledge Base</h3>
              

              <div className="space-y-1 max-h-48 overflow-y-auto">
                {documents.length === 0 ? (
                  <p className="text-xs text-slate-400 dark:text-slate-500 italic px-2 whitespace-nowrap">No documents uploaded</p>
                ) : (
                  documents.map((doc, idx) => {
                    ...
                    return (
                      ...
                      >
                        <div 
                          className="flex items-center gap-2 truncate flex-1 cursor-pointer hover:opacity-80 whitespace-nowrap"
            ...
            <div>
              <h3 className="text-xs font-semibold text-slate-400 dark:text-slate-500 mb-2 uppercase tracking-wider whitespace-nowrap">Recent Chats</h3>
              <div className="space-y-1">
                {conversations.length === 0 ? (
                  <p className="text-xs text-slate-400 dark:text-slate-500 italic px-2 whitespace-nowrap">No recent chats</p>
                ) : (
                  conversations.map(conv => (
                    ...
                    >
                      <div className="flex items-center gap-2 truncate pr-2 whitespace-nowrap">
